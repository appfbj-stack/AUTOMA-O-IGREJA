...
export default function LogsPage() {
  ...
  useEffect(() => { getLogs(200).then(setLogs); }, []);

  const filtered = logs.filter(l => filter === 'todos' || l.resultado === filter);
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-20">
      ...
      <main className="px-4 py-4 max-w-lg mx-auto">
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-slate-500"><p className="text-3xl mb-2">📋</p><p>Nenhum log registrado.</p></div>
        ) : (
