...
const emptyForm = (): Omit<Dispositivo, 'id' | 'status'> => ({ nome: '', tipo: 'sonoff', ip: '', porta: 80 });
...
export default function DispositivosPage() {
  ...
  async function loadDevices() { setDispositivos(await getDispositivos()); }
  ...
  async function handleDelete(id: string) {
    ...
    await deleteDispositivo(id); await loadDevices();
  }
        ...
        {showForm && (
          <div className="bg-slate-800 rounded-2xl p-4 space-y-3 border border-slate-600">
            ...
            <div>
              ...
              <input className="w-full mt-1 bg-slate-700 text-white rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
                value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Ex: Sonoff Luz Altar" />
            </div>
            <div>
              <label className="text-xs text-slate-400">Tipo</label>
              <select className="w-full mt-1 bg-slate-700 text-white rounded-xl px-3 py-2 text-sm outline-none"
                value={form.tipo} onChange={e => setForm(f => ({ ...f, tipo: e.target.value as Dispositivo['tipo'] }))}>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="col-span-2">
                ...
                <input className="w-full mt-1 bg-slate-700 text-white rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
                  value={form.ip} onChange={e => setForm(f => ({ ...f, ip: e.target.value }))} placeholder="192.168.1.x" />
              </div>
              <div>
                <label className="text-xs text-slate-400">Porta</label>
                <input type="number" className="w-full mt-1 bg-slate-700 text-white rounded-xl px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
                  value={form.porta} onChange={e => setForm(f => ({ ...f, porta: Number(e.target.value) }))} />
              </div>
            </div>
            <div className="flex gap-2 pt-1">
              <button onClick={handleSave} className="flex-1 bg-brand-500 hover:bg-brand-600 text-white font-medium py-2 rounded-xl text-sm">Salvar</button>
              <button onClick={() => { setShowForm(false); setEditingId(null); }} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-medium py-2 rounded-xl text-sm">Cancelar</button>
            </div>
          </div>
        )}

        {dispositivos.length === 0 && !showForm ? (
          <div className="text-center py-12 text-slate-500"><p className="text-3xl mb-2">📡</p><p>Nenhum dispositivo cadastrado.</p></div>
        ) : (
          dispositivos.map(d => (
            <div key={d.id} className="bg-slate-800 rounded-2xl px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 mt-0.5">{tipoLabels[d.tipo]} · {d.ip}:{d.porta}</p>
              </div>
