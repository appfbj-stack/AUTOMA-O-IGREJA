...
export default function AutomacoesPage() {
  ...
  useEffect(() => { getAutomacoes().then(setAutomacoes); }, []);
  ...
  async function run(id: string) {
    ...
    try {
      ...
      setResultado({ id, ok: erros.length === 0, msg: erros.length === 0 ? `${resultados.length} ação(ões) executadas` : `${erros.length} erro(s): ${erros.map(e => e.tipo).join(', ')}` });
    } catch (e: unknown) {
  ...
  const icons: Record<string, string> = { 'iniciar-culto': '▶️', 'modo-louvor': '🎵', 'modo-pregacao': '📖', 'encerrar-culto': '⏹️' };
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-20">
      ...
      <main className="px-4 py-6 max-w-lg mx-auto space-y-3">
        {resultado && (
          <div className={`rounded-xl px-4 py-3 text-sm font-medium ${resultado.ok ? 'bg-emerald-900/40 border border-emerald-700 text-emerald-300' : 'bg-red-900/40 border border-red-700 text-red-300'}`}>
          ...
          <div key={a.id} className="bg-slate-800 rounded-2xl p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                ...
                {a.descricao && <p className="text-xs text-slate-400 mt-1 ml-9">{a.descricao}</p>}
                <div className="mt-2 ml-9 flex flex-wrap gap-1">
                  {a.acoes.map((ac, i) => (
                    <span key={i} className="text-xs bg-slate-700 text-slate-400 px-2 py-0.5 rounded-full">{ac.tipo}</span>
                  ))}
                </div>
              </div>
