...
export default function SomPage() {
  ...
  async function applyPreset(nome: string) {
    ...
    try {
      ...
      await addLog({ data: new Date().toISOString(), acao: `XR12 Preset: ${nome}`, usuario, resultado: 'sucesso' });
      setSuccess(`Preset "${nome}" aplicado!`);
      setTimeout(() => setSuccess(''), 3000);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      await addLog({ data: new Date().toISOString(), acao: `XR12 Preset: ${nome}`, usuario, resultado: 'erro', detalhes: msg });
      setError(msg);
    } finally {
      setLoading(null);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-20">
      ...
      <main className="px-4 py-6 max-w-lg mx-auto space-y-4">
        ...
        {success && (
          <div className="bg-emerald-900/40 border border-emerald-700 text-emerald-300 rounded-xl px-4 py-3 text-sm">
            ✅ {success}
          </div>
        )}

        <p className="text-slate-400 text-sm">Snapshots do Behringer XR12 (porta OSC 10024):</p>
        ...
        {presets.map(p => (
          <BigButton
            ...
            disabled={!!loading && loading !== p.id}
            onClick={() => applyPreset(p.id)}
          />
        ))}

        <div className="bg-slate-800 rounded-2xl p-4 mt-4">
          <h2 className="text-sm font-semibold text-slate-400 mb-3">Canais individuais</h2>
          <p className="text-slate-500 text-sm text-center py-4">
            Configure os canais na página de Dispositivos
          </p>
        </div>
      </main>
