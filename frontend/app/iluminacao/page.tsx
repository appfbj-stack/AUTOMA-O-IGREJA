...
export default function IluminacaoPage() {
  ...
  async function toggle(dispositivo: Dispositivo) {
    ...
    try {
      ...
      await addLog({ data: new Date().toISOString(), acao: `Luz ${dispositivo.nome}: ${on ? 'LIGOU' : 'DESLIGOU'}`, usuario: 'operador', resultado: 'sucesso' });
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      await addLog({ data: new Date().toISOString(), acao: `Luz ${dispositivo.nome}: erro`, usuario: 'operador', resultado: 'erro', detalhes: msg });
    } finally {
      setLoading(null);
    }
  }
              ...
              return (
                <button key={d.id} onClick={() => toggle(d)} disabled={loading === d.id}
                  className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl transition-all active:scale-[0.98] disabled:opacity-50 ${
                    on ? 'bg-amber-500/20 border border-amber-500/40' : 'bg-slate-800 border border-slate-700'
                  }`}
                >
